...
<?php
    ...
    function formatPickupDate($order_date){
        $dateTime = new DateTime($order_date);
        ...
        $totalOrders = $orderObj->getTotalOrders($keyword,$status,'pickup');
        $totalPages = ceil($totalOrders / $ordersPerPage);

        $recent_orders = $orderObj->showAllOrders($keyword,$status, 'pickup','',$start, $ordersPerPage);
?>
    ...
    <div class="main-content">
        ...
        <div class="flex gap-5 flex-wrap items-center">
            <div class="bg-[#1e1e1e] py-[30px] px-[20px] rounded-[12px] shadow-2xl flex flex-col items-start transition-transform duration-300 my-3 w-[250px] gap-3" >
                    ...
                    <p class=" text-xl "><?= $orderObj->getOrdersByMonth(idate("m"), '','pickup') ?></p>
            </div>
            <div class="bg-[#1e1e1e] py-[30px] px-[20px] rounded-[12px] shadow-2xl flex flex-col items-start transition-transform duration-300 my-3 w-[250px] gap-3" >
                <p class=" text-xs bg-white/10 text-white  rounded-full py-1 px-2">monthly</p>
                <p class=" text-2xl text-customOrange">Pending</p>
                <p class=" text-xl "><?= $orderObj->getOrdersByMonth(idate("m"), 'pending','pickup') ?></p>
        
            </div>
            <div class="bg-[#1e1e1e] py-[30px] px-[20px] rounded-[12px] shadow-2xl flex flex-col items-start transition-transform duration-300 my-3 w-[250px] gap-3" >
                <p class=" text-xs bg-white/10 text-white  rounded-full py-1 px-2">monthly</p>
                <p class=" text-2xl text-customOrange">Cancelled</p>
                <p class=" text-xl "><?= $orderObj->getOrdersByMonth(idate("m"), 'cancelled','pickup') ?></p>
            </div>
            <div class="bg-[#1e1e1e] py-[30px] px-[20px] rounded-[12px] shadow-2xl flex flex-col items-start transition-transform duration-300 my-3 w-[250px] gap-3" >
                <p class=" text-xs bg-white/10 text-white  rounded-full py-1 px-2">monthly</p>
                <p class=" text-2xl text-customOrange">Completed</p>
                <p class=" text-xl "><?= $orderObj->getOrdersByMonth(idate("m"), 'completed','pickup') ?></p>
            </div>
        </div>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?= formatPickupDate($order['pickup_date']) ?></td>
                        <td><?= $order['status'] ?></td>
